...
def my_euclidean_dist(X_test, X_train):
    ...
    a = [x ** 2 + y ** 2 for x,y in X_test]
    b = [x ** 2 + y ** 2 for x,y in X_train]
    for i, test2 in enumerate(X_test):
        for j, train2 in enumerate(X_train):
...
#my_dist = my_euclidean_dist(X_test, X_train)
#print (my_dist)
...
def split_nfold(num_examples, n):
    ...
    for i in range(n):
        ...
        exclude = [idx[j] for j in range(start, end)]
        train_set = [idx[j] for j in range(num_examples-1) if not idx[j] in exclude]
        train_sets.append(train_set)
        val_set = [idx[k] for k in range(start, end)]
        validation_sets.append(val_set)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return train_sets, validation_sets
    
#train_sets, val_sets = split_nfold(11,5)
#print("train and val sets")
#print(train_sets[4])
#print(val_sets[4])
...
def problem2():
    ...
    for t in range(trials):
        trial_size = trial_sizes[t]
        temp = [Xtrain[i] for i in range(trial_size)]
        trial_X = np.array(temp)
        temp = [ytrain[i] for i in range(trial_size)]
        trial_Y = np.array(temp)
